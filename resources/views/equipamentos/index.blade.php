...
<div class="space-y-6">
    ...
    <form method="GET" class="panel p-4 flex flex-wrap items-end gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="stat-card">
            ...
            <p class="text-3xl font-extrabold mt-1 text-slate-900">{{ $totalGeral }}</p>
        </div>
        <div class="stat-card">
            <p class="text-sm font-medium text-slate-500">Total por cidade</p>
            <p class="text-3xl font-extrabold mt-1 text-slate-900">{{ $totalCidade }}</p>
            @if($filters['cidade_comarca_id'] ?? false)
                <p class="text-xs text-cyan-600 mt-1">{{ $cidades->find($filters['cidade_comarca_id'])?->nome }}</p>
            @endif
        </div>
        <div class="stat-card">
            <p class="text-sm font-medium text-slate-500">Total por vara</p>
            <p class="text-3xl font-extrabold mt-1 text-slate-900">{{ $totalVara }}</p>
            @if($filters['vara_id'] ?? false)
                <p class="text-xs text-cyan-600 mt-1">{{ $varas->find($filters['vara_id'])?->nome }}</p>
            @endif
        </div>
    </div>
    ...
    <div class="space-y-3 md:hidden">
        @forelse ($equipamentos as $eq)
            <article class="panel p-4 hover:shadow-lg transition-shadow">
