...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <article class="stat-card panel-hover group">
            <div class="flex items-start justify-between">
                <div>
                    <p class="text-sm font-medium text-slate-500">Total de equipamentos</p>
                    <p class="text-4xl font-extrabold mt-2 bg-gradient-to-r from-slate-900 to-slate-600 bg-clip-text text-transparent">{{ $totalEquipamentos }}</p>
                </div>
        ...
        <article class="md:col-span-2 panel p-5">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                @foreach ($totaisPorTipo as $item)
                    <div class="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-cyan-200 hover:bg-cyan-50/50 transition-all duration-200">
                        <span class="text-sm font-medium text-slate-700 truncate">{{ $item->nome }}</span>
                        <span class="flex-shrink-0 px-2.5 py-0.5 rounded-full text-xs font-bold bg-cyan-100 text-cyan-800">{{ $item->equipamentos_count }}</span>
                    </div>
                @endforeach
            </div>
        </article>
    ...
    <article class="panel p-5">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            @foreach ($totaisPorCidade as $item)
                <a href="{{ route('equipamentos.index', ['cidade_comarca_id' => $item->id]) }}" class="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-cyan-300 hover:bg-cyan-50/70 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 group">
                    <span class="text-sm font-medium text-slate-700 truncate group-hover:text-cyan-900">{{ $item->nome }}</span>
                    <span class="flex-shrink-0 px-2.5 py-0.5 rounded-full text-xs font-bold bg-slate-200 text-slate-700 group-hover:bg-cyan-100 group-hover:text-cyan-800">{{ $item->equipamentos_count }}</span>
                </a>
            @endforeach
        </div>
    </article>
    ...
    <article class="panel p-5">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('equipamentos.index') }}" class="text-sm text-cyan-600 hover:text-cyan-800 font-medium">Ver todos →</a>
        </div>
        <div class="overflow-x-auto -mx-5 px-5">
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-200">
                        <th class="py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Patrimônio</th>
                        <th class="py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Tipo</th>
                        <th class="py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Cidade</th>
                        <th class="py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Setor</th>
                        <th class="py-3 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider"></th>
                    </tr>
                </thead>
                <tbody>
